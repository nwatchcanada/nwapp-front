...
function fileFormatter(cell, row){
    return (
        <div>
            ...
            &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;
            {row.isArchived === false &&
                <Link to={`/admin/settings/resource/${row.user}/file/archive/${row.slug}`}>
...
function detailLinkFormatter(cell, row){
    if (row.isArchived === true) {
        ...
        return (
            <div>
                <Link to={`/admin/settings/resource/${row.slug}`} className="btn btn-success btn-xs">
                    <i className="fas fa-binoculars"></i>&nbsp;View
                </Link>;
                <Link to={`/admin/settings/resource/operation/archive/${row.slug}`} className="btn btn-danger btn-xs">
                    <i className="fas fa-archive"></i>&nbsp;Archive
                </Link>
            </div>
        )
...
class AdminResourceListComponent extends Component {
    render() {
        const {
            // Pagination
            page, sizePerPage, totalSize,
        ...
        return (
            <div>
                ...
                <h1><i className="fas fa-bullhorn"></i>&nbsp;Resource</h1>
                        ...
                        <section className="row text-center placeholders">
                            <div className="col-sm-12 placeholder">
                                <div className="rounded-circle mx-auto mt-4 mb-4 circle-200 bg-pink">
                                    <Link to={`/admin/settings/resource/add/step-1`} className="d-block link-ndecor" title="Members">
                                        <span className="r-circle"><i className="fas fa-plus fa-3x"></i></span>
                                    </Link>
                                </div>
                                <h4>Add Resource</h4>
                                <div className="text-muted">Add Resource</div>
                            </div>
                        </section>
                    </div>
                </div>
...
export default AdminResourceListComponent;
