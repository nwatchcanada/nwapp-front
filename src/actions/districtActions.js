...
import {
    ...
    NWAPP_DISTRICT_BOUNDRY_OPERATION_API_ENDPOINT
} from '../constants/api';
...
export function putDistrictBoundryOperation(putData, successCallback, failedCallback) {
    return dispatch => {
        // Change the global state to attempting to log in.
        store.dispatch(
            setDistrictDetailRequest()
        ...
        // Perform our API submission.
        customAxios.put(NWAPP_DISTRICT_BOUNDRY_OPERATION_API_ENDPOINT.replace("<slug>", putData.slug), buffer).then( (successResponse) => {
            // Decode our MessagePack (Buffer) into JS Object.
            const responseData = msgpack.decode(Buffer(successResponse.data));
            let device = camelizeKeys(responseData);
            ...
            if (exception.response) {
                ...
                console.log("putDistrictBoundryOperation | error:", errors); // For debuggin purposes only.
