...
import AdminTaskSearchComponent from "../../../../components/taskItems/admin/search/searchComponent";
import { validateSearchInput } from "../../../../validators/taskValidator";
import { localStorageSetObjectOrArrayItem } from '../../../../helpers/localStorageUtility';


class AdminTaskSearchContainer extends Component {
    /**
     *  Initializer & Utility
     *------------------------------------------------------------
     */
	...
	handleKeyDown(e) {
		...
		if (e.keyCode === 13) {
			this.setState({ advancedSearchActive: false, }, ()=> {
				// Perform task-side validation.
				const { errors, isValid } = validateSearchInput(this.state);

				// CASE 1 OF 2: Validation passed successfully.
				if (isValid) {

					localStorageSetObjectOrArrayItem('workery-search-task-details', this.state);
